...
%% real auth-flow data
data_name = 'metro_flows';
real_graphs = importdata('metro_flows_edges_real.txt');
data_name = 'metro_flows';
diary DSBM_run_metro_flows.log;
diary on;
n_times = 24;
n_nodes = 91;
n_known_class = 1;
n_unkown_class = 2;
roles=[1:n_nodes]';
roleLabels=permute(roles,[2,1]);
train_ratio = 0.01;
nSynNets = 100;

%% real auth-flow data
% data_name = 'auth_flows';
% real_graphs = importdata('auth_flows_edges_real.txt');
% data_name = 'auth_flows';
% diary DSBM_run_auth_flows.log;
% diary on;
% n_times = 10;
% n_nodes = 9;
% n_known_class = 1;
...
%% real auth data
% data_name = 'auth';
% real_graphs = importdata('auth_user_0.txt');
% data_name = 'auth';
% diary DSBM_run_auth.log;
% diary on;
% n_times = 4;
% n_nodes = 27;
% n_known_class = 1;
...
%% real metro data
% data_name = 'metro';
% real_graphs = importdata('metro_user_4.txt');
% data_name = 'metro';
% diary DSBM_run_metro.log;
% diary on;
% n_times = 6;
% n_nodes = 91;
% n_known_class = n_nodes;
% n_unkown_class = 2;
% roles=[1:n_nodes]';
% roleLabels=permute(roles,[2,1]);
% train_ratio = 0.06;
...
% real_graphs = importdata('data-scale-free-nodes-500-samples-100.txt');
% data_name = 'scale-free-nodes-500';
% diary DSBM_run_scale-free-nodes-500.log;
% diary on;
% n_times = 5;
% n_nodes = 500;
% n_known_class = n_nodes;
...
%% real protein data
% data_name = 'protein';
% real_graphs = importdata('protein_100_real.txt');
% data_name = 'protein100';
% diary DSBM_run_protein100.log;
% diary on;
% n_times = 100;
% n_nodes = 8;
% n_known_class = n_nodes;
...
t_col = 4;
directed = 1;
if ~strcmp(data_name, 'auth_flows') && ~strcmp(data_name, 'metro_flows')
    time_interval = 1. / n_times;
    real_graphs(:, t_col) = ceil(real_graphs(:, t_col) / time_interval);
else
    real_graphs(:, t_col) = real_graphs(:, t_col) + 1;
end
...
unique_d = unique(real_graphs(:, d_col));
...
eye_matrix = [];
for t = 1:n_times
    eye_matrix = cat(3, eye_matrix, eye(n_nodes));
end

train_inx = int32(floor(d_size * train_ratio));
disp(['number of graphs ' mat2str(train_inx)]);

% for i = 1:1
for i = 1:train_inx
    d = unique_d(i);
    ind = real_graphs(:, d_col) == d;
    graph = real_graphs(ind, :);
    graph = graph(:, i_col:t_col);
    d_adj = networksFromContacts(graph, n_nodes, n_times, directed);
    d_adj = d_adj + eye_matrix;
    adj = cat(3, adj, d_adj);
end
...
% time_start = string(datetime('now'));
now1 = tic();

% if matlabpool('size') == 0 % for matlab 2009b version
%     matlabpool open 2
% end
...
wholeTime = toc(now1);
disp('running time of DSBM model');
disp(wholeTime);
...
% parfor iSyn = 1:nSynNets
    ...
    adjSbtm_fake_graphs = cat(3, adjSbtm_fake_graphs, adjSbtm);
    adjDsbm_fake_graphs = cat(3, adjDsbm_fake_graphs, adjDsbm);
%     disp(['adjSbtm', mat2str(size(adjSbtm))]);
%     disp(['adjDsbm', mat2str(size(adjDsbm))]);
end

diary off;

file_name = ['DSBM_' data_name '_' datestr(datetime(), 'mmm-dd-yyyy_HH-MM-SS') '.mat'];
save(file_name, 'adjSbtm_fake_graphs', 'adjDsbm_fake_graphs'); 
...
% title(['Estimated edge probability from ' roleLabels(2) 's to ' ...
%     roleLabels(col) 's'])
% 
